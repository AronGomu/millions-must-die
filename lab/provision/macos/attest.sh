#!/usr/bin/env bash
# macOS ref host inspect + contract check (dry-run via fixtures; live inspect later).
...
macOS runner manifest via trusted mmd-lab (or cargo bin under test).
...
MANIFEST="${ROOT}/lab/manifests/macos-15-arm64.toml"
LAB_BIN="${MMD_LAB_BIN:-}"
DRY_RUN=0
FIXTURE=""
...
if [[ "$DRY_RUN" -ne 1 ]]; then
  echo "live host inspect not implemented (T22). use --dry-run --fixture <json>" >&2
  exit 2
fi
...
exec "$LAB_BIN" attest-macos --manifest "$MANIFEST" --observed "$FIXTURE"
