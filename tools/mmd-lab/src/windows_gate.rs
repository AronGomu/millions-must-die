//! Windows candidate gate lane (T20). Coordinator-side adapter, thin by design
//! — a deliberate mirror of the Ubuntu lane (T17), not a cross-platform
//! abstraction.
//!
//! Lane order (each step gates the next):
//! 1. Host attestation — identity only (contract match; never perf truth).
//!    Microsoft Basic Render Driver / WARP is a hard reject here.
//! 2. Recovery protocol must be `ReadyForCandidate` (external WinPE/FFU lane).
...
//!
//! Fixture/fake-transport scope: the native D3D12 smoke/scale run and the
//! physical WinPE/FFU reset are deferred-hw; this lane drives the exact same
//! coordinator protocol against fixture evidence. The committed golden for
//! `windows-d3d12` remains an honest placeholder — the fixture lane binds to a
...
use mmd_engine::render::{
    BACKEND_D3D12, HostBinding, compare_readback, decode_readback_png, load_golden_image,
...
use crate::windows::{
    WindowsAttestVerdict, WindowsHostAttestation, WindowsRunnerManifest,
    validate_windows_attestation,
};
use crate::windows_recovery::{WindowsRecoveryEvent, WindowsRecoveryState};

/// Runner-contract backend id (T18 naming); golden manifests use the SDL
/// driver name [`BACKEND_D3D12`] (`direct3d12`).
const CONTRACT_BACKEND_D3D12: &str = "d3d12";

/// Inputs frozen before dispatch (trusted side; never candidate-supplied).
pub struct WindowsGateParams<'a> {
    pub runner_manifest: &'a WindowsRunnerManifest,
    /// Observed host attestation (identity only).
    pub attestation: &'a WindowsHostAttestation,
...
/// External post-run reset hook. Real WinPE/FFU power control is deferred-hw;
/// the protocol event below is the trusted-side contract either way.
pub trait WindowsResetController {
    /// Initiate external restore after a candidate run. `Ok` = protocol
    /// restore started (host leaves candidate-ready state).
    fn start_post_run_restore(&mut self, state: &mut WindowsRecoveryState) -> Result<(), String>;
}

/// Protocol-level controller: applies the external-restore event.
pub struct WindowsProtocolResetController;

impl WindowsResetController for WindowsProtocolResetController {
    fn start_post_run_restore(&mut self, state: &mut WindowsRecoveryState) -> Result<(), String> {
        state.apply(WindowsRecoveryEvent::StartExternalRestore {
            external_controller: true,
        })
    }
}

/// Run the Windows candidate gate lane. See module docs for order + risk.
pub fn run_windows_gate(
    params: &WindowsGateParams<'_>,
    recovery: &mut WindowsRecoveryState,
    transport: &mut dyn AgentTransport,
    archive: &ArchiveBlob,
    readback_png: &[u8],
    reset: &mut dyn WindowsResetController,
) -> WindowsGateOutcome {
    let mut trail = vec!["attest-identity".to_string()];

    // 1. Host attestation — identity only; refusal means no dispatch at all.
    //    Basic Render Driver / WARP arrives here as WindowsAttestVerdict::Reject.
    let attest = validate_windows_attestation(params.runner_manifest, params.attestation);
    if attest.verdict != WindowsAttestVerdict::ReadyForRecovery {
        return WindowsGateOutcome::refused(
            format!(
...
/// perf truth stays in `verify_host`). Non-D3D12 evidence is rejected here.
fn evidence_matches_contract(
    manifest: &WindowsRunnerManifest,
...
fn golden_diff(
    ...
    let binding = HostBinding {
        backend: golden_backend(&evidence.host_manifest.backend),
...
/// T18 runner contracts pin backend `d3d12`; golden manifests bind the SDL
/// driver name `direct3d12`. Map only that exact id; anything else passes
/// through untouched. Non-contract evidence backends (including a raw
/// `direct3d12`) are rejected by `evidence_matches_contract` against the
/// frozen `d3d12` pin, so the lane still fails honestly either way.
fn golden_backend(contract_backend: &str) -> String {
    if contract_backend == CONTRACT_BACKEND_D3D12 {
        BACKEND_D3D12.to_string()
    } else {
        contract_backend.to_string()
    }
}

/// `windows-x86_64` → `windows` (golden manifests bind an OS family).
...
mod tests {
    ...
    use crate::windows::{parse_attestation_json, parse_manifest_toml};
    use crate::windows_recovery::simulate_successful_windows_drill;

    const FFU_DIGEST: &str = "cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc";
    const TEST_ADAPTER: &str = "Test GPU RX 6400";
    const ATLAS_PIN: &str = "atlas-pin";
    const SHADER_PIN: &str = "shader-pin";

    fn runner_manifest() -> WindowsRunnerManifest {
...
"#
    ...
    fn attestation_pass() -> WindowsHostAttestation {
  ...
  "d3d12_adapter_type": "discrete_gpu"
    ...
    fn pass_evidence() -> HostEvidence {
        let mut manifest =
            HostManifest::new("windows-ref", "windows-x86_64", "d3d12", "Windows 11 25H2");
    ...
    struct Lane {
        ...
        golden_dir: PathBuf,
        manifest: WindowsRunnerManifest,
        attestation: WindowsHostAttestation,
        archive: ArchiveBlob,
        policy: BenchPolicy,
    }
    ...
    impl Lane {
        fn new() -> Self {
            ...
            let golden_dir = tmp.path().join("windows-d3d12");
        ...
        fn params(&self) -> WindowsGateParams<'_> {
            WindowsGateParams {
        ...
        fn run(
            ...
            readback: &[u8],
            reset: &mut dyn WindowsResetController,
        ) -> (WindowsGateOutcome, WindowsRecoveryState) {
            let mut recovery =
                simulate_successful_windows_drill(FFU_DIGEST, "WIN-MACHINE-FRESH", None);
    ...
    impl WindowsResetController for DenyReset {
        fn start_post_run_restore(
            &mut self,
            _state: &mut WindowsRecoveryState,
        ) -> Result<(), String> {
    ...
    #[test]
    fn windows_tampered_stats_fail() {
        let lane = Lane::new();
    ...
    #[test]
    fn windows_wrong_backend_fails() {
        let lane = Lane::new();
        let mut evidence = pass_evidence();
        ...
        assert!(
            outcome
                .reasons
                .iter()
                .any(|r| r.contains("evidence backend vulkan != contract d3d12")),
            "{outcome:?}"
        );
        // Non-D3D12 evidence also fails the golden backend binding.
        assert!(!outcome.golden_ok, "{outcome:?}");
    }

    #[test]
    fn windows_wrong_archive_fails() {
        let lane = Lane::new();
        let (outcome, _) = lane.run(
            pass_evidence(),
    ...
    #[test]
    fn windows_reset_after_run_required() {
        let lane = Lane::new();
        let (outcome, recovery) = lane.run(
            pass_evidence(),
            false,
    ...
    #[test]
    fn windows_wrong_archive_and_denied_reset_escalates() {
        let lane = Lane::new();
        let (outcome, _) = lane.run(
            pass_evidence(),
            true, // delivery fails
    ...
    #[test]
    fn windows_basic_renderer_reject_stops_before_dispatch() {
        let lane = Lane::new();
        let mut attestation = attestation_pass();
        attestation.d3d12_adapter_name = "Microsoft Basic Render Driver".into();
        attestation.d3d12_adapter_type = "basic".into();
        attestation.gpu_model = "Microsoft Basic Render Driver".into();
        let params = WindowsGateParams {
            attestation: &attestation,
            ..lane.params()
        };
        let mut recovery = simulate_successful_windows_drill(FFU_DIGEST, "WIN-MACHINE-FRESH", None);
        ...
        assert!(
            outcome
                .reasons
                .iter()
                .any(|r| r.contains("basic render driver rejected")),
            "{outcome:?}"
        );
    }

    #[test]
    fn windows_recovery_not_ready_stops_before_dispatch() {
        let lane = Lane::new();
        let mut recovery = WindowsRecoveryState::new(FFU_DIGEST); // Idle, never restored
        let mut agent = FakeAgent::from_evidence(pass_evidence());
        let outcome = run_windows_gate(
            &lane.params(),
            &mut recovery,
