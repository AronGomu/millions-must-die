...
- [ ] With that same non-16:9 window, press and hold the left button *in a bar*, move the pointer into the scene content and release it over a unit, the HUD or a pending building ghost: confirm nothing happens — no selection, no HUD activation, no placement. Repeat with Shift held and with the right button (release over a unit while workers are selected): confirm no order and no ghost cancel. Then repeat each gesture with press *and* release fully inside the content: confirm they select, activate and order normally.
...
- [ ] Force a save failure (e.g. remove write permission on the settings directory, or point `SDL_VIDEODRIVER`'s pref dir at a read-only path) and change a setting: confirm a `SETTINGS NOT SAVED: <reason>` line appears in the settings panel, the control visibly reverts to its old value, the old volume/window behavior is actually back, and the session continues normally. That banner is only correct because the rollback itself succeeded.
- [ ] Not manually testable, and deliberately so: the *failed compensation* path (a rollback that itself fails, leaving the mixer or window on values no config describes). Injecting a double native failure is unsafe and non-portable, so it is proven by automated fakes only — no manual fatal injection has been run. Expected live behavior if it ever happens: no banner, the GPU claim released and the window torn down, and exit 1 naming the primary failure, every failed compensation, and any reclaim/lifecycle context.
...
`artifacts/PLAN_2026_08_14_rts-feedback-polish/T14_gather-exit-separation.md`.
