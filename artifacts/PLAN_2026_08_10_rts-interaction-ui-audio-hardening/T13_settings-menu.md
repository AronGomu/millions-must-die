...
- [ ] manual check: gear→Settings; edit each control; relaunch; values persist; Escape nesting correct — queued in `artifacts/manual_test_checklist.md`'s new `T13 settings-menu` section (no real window/pointer grab from this worker, per harness constraint).
