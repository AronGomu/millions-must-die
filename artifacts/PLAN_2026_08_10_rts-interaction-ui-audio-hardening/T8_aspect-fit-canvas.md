...
- [ ] manual check: resize non-16:9 window → no stretch; bar click causes no world action — **unchecked in the review-fix pass: this needs a real window and nobody opened one.** The equivalent human steps live in `artifacts/manual_test_checklist.md` § T8 (resize to 1280x1024 or an ultrawide, confirm no stretch and flat bars; click in a bar and confirm no world action).
- [ ] app functional: `cargo run -- rts --frames 60` — **unchecked for the same reason: without `SDL_VIDEODRIVER=offscreen` this opens a real window.** The offscreen equivalent is on the merge gate; the windowed one is `artifacts/manual_test_checklist.md` § T8.
