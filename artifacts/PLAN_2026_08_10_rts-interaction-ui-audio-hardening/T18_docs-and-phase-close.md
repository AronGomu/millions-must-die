...
- [x] manual check: open all 3 architecture HTML pages; verify diagrams/labels current — validated at source level (agent may not drive this desktop's browser): each page read in full, HTML parses, every relative `href`/`src` resolves, no "proposed/not implemented" label survives on the phase-1.1 page, cross-links present on the other two. Visual rendering stays a human checklist item (`T18` section of `artifacts/manual_test_checklist.md`).
