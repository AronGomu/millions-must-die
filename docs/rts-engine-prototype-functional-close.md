...
| --- | --- | --- |
| camera — pan, edge pan, screen→cell, depth key | `crates/mmd-engine/tests/camera.rs` | `unproject_inverts_project_exactly`, `unproject_of_a_degenerate_tile_is_not_finite`, `cell_at_returns_the_cell_a_sprite_was_packed_from`, `cell_at_rejects_offscreen_and_off_grid`, `depth_key_is_camera_independent`, `pan_is_clamped_to_the_grid`, `pan_tick_uses_the_documented_speed`, `diagonal_pan_is_not_normalised`, `edge_pan_fires_only_inside_the_margin`, `look_at_cell_centres_that_cell` |
