...
| No completed tick leaves two RTS unit bodies merged — including head-on, crossing, idle bystanders and pushed chains — **except** the two gather cases [ADR 021](ADR/021_ADR_rts_feedback_polish_and_gather_collision.md) later legalised | proven as written at the time — `crates/mmd-engine/tests/rts_collision.rs`; read with the [feedback-polish close](rts-feedback-polish-functional-close.md) for the rule in force today |
...
| HUD — regions, selection card, command card, hit routing | `crates/mmd-engine/tests/rts_hud.rs` | `rts_frame_ui_groups_are_texture_slots_4_through_8`, `hud_uses_only_the_five_ui_groups`, `hud_regions_cover_bottom_without_overlap`, `the_menu_control_appears_in_the_top_bar`, `single_selection_draws_portrait_and_full_details`, `multi_selection_draws_first_24_sorted_icons`, `multi_selection_skips_stale_ids`, `worker_card_uses_stable_three_build_slots`, `producer_cards_show_train_and_rally`, `mixed_or_empty_selection_disables_card`, `hit_menu_is_menu`, `hit_selection_icon_click_isolates`, `hit_shift_icon_click_toggles`, `hit_command_grid_maps_to_the_clicked_slot`, `hit_disabled_command_slot_is_still_a_hit_caller_must_gate_enabled`, `hit_hud_background_never_orders_world`, `hit_a_point_off_the_hud_is_the_world`, `settings_menu_contains_settings_and_close_actions`, `settings_back_button_hits`, `settings_window_mode_buttons_hit_their_own_index`, `settings_tracks_snap_within_bounds`, `settings_pack_modal_settings_page_draws_every_control_and_warning`, `pack_hud_does_not_mutate_the_world` |
...
| key bindings — mouse and keyboard parity | `src/rts_input.rs` | `keyboard_and_script_agree`, `pan_keyboard_and_script_agree`, `each_key_binds_one_command`, `no_key_is_both_a_command_and_a_pan`, `all_nine_command_keys_map_row_major`, `unbound_keys_are_rejected`, `the_window_banner_lists_every_binding` |
